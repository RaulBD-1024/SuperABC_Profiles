...
Con colaboración de: Camila Estrada Alfaro, Daniela Estrada Alfaro y Leonardo Vargas Piedra
...
### 📦 **6. Registros y optimización de almacén:**
...
with st.sidebar:
    ...
    unit_vol = st.selectbox('Unidad de volumen', ['pies3 (p3)','metros3 (m3)'], help = 'Selecciona la unidad en que está el volumen en tu archivo Excel')
...
# Guardar base en session_state para usarlo en perfiles
...
if 'by_item' in st.session_state:
    ...
    with st.expander("📈 Perfiles de Actividad", expanded=False):
        ...
        st.subheader('% de órdenes por rango de volumen (ft³)')

        # Calcular volumen total por orden
        cubic_per_order = base.groupby('NumDoc').agg(volumen_total=('Volumen_m3','sum')).reset_index()

        # Convertir volumen de m³ a ft³
        cubic_per_order['volumen_total_ft3'] = cubic_per_order['volumen_total'] * 35.3147

        # Definir rangos en pies cúbicos
        vol_bins = [-1, 18, 35, 70, 175, 350, 700, 1750, 1e9]
        vol_labels = ['≤18', '18-35', '35-70', '70-175', '175-350', '350-700', '700-1750', '>1750']

        # Asignar categoría de volumen por orden
        cubic_per_order['vol_bin'] = pd.cut(
            cubic_per_order['volumen_total_ft3'],
            bins=vol_bins,
            labels=vol_labels
        )
        ...
        fig_cubic = px.bar(
            ...
            x='vol_bin',
            y='%_ordenes',
            labels={'vol_bin': 'Rango volumen (ft³)', '%_ordenes': '% de órdenes'}
        )
        ...
        lv['volumen_total'] = lv['volumen_total'] * 35.3147
        
        # 🔹 Nuevos rangos en pies cúbicos
        vol_labels = ['≤17.7', '17.7-35.3', '35.3-70.6', '70.6-176.6', '176.6-353.1', '353.1-706.3', '706.3-1765.7', '>1765.7']
        lv['r_vol'] = pd.cut(lv['volumen_total'], bins=[-35.31, 17.66, 35.31, 70.63, 176.57, 353.15, 706.29, 1765.73, 1e9], labels=vol_labels, right=True, include_lowest=True)
        ...
        # -------------------------------
        # Gráfico de pastel interactivo (por volumen total)
        # -------------------------------
        ...
        # -------------------------------
        # Heatmap interactivo (líneas x volumen)
        # -------------------------------
        ...
        fig_heatmap.update_layout(
            ...
            xaxis_title='Volumen por orden (p³)',
...
# =============================
# Análisis y Optimización de Almacén
# =============================
st.header('⚙️ Análisis y Optimización de Almacén')
